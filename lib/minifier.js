...
var REGEXP_IMAGE = exports.REGEXP_IMAGE = /url\s*\(\s*["']?\s*([^\(\)]*)\.(cur|png|jpg|jpeg|gif|webp)\s*["']?\s*\)/gi;
var LINE_BREAK = os.EOL;
...
exports.optimage = function(input, output, callback, level, verbose, options){
  ...
  doOptimize(function(){
    if(fs.existsSync(tmpOutput) && imageIsEnhanced(input, tmpOutput)){
...
@param {Object} options 压缩配置项
    ...
    @param {Array} options.remove 移除掉log信息的target对象字符串，默认是['console']
    @param {Array} options.ascii_only 是否将中文转换成unicode编码，默认true
    @param {String} options.banner 压缩后的文件头部自定义加入的字符串
    @param {String} options.fotter 压缩后的文件尾部自定义加入的字符串
...
exports.minifyJS = function (filecontent, options) {
  ...
  /*fixed
  //bugfix for the issue: https://github.com/mishoo/UglifyJS2/issues/242
  ...
  UglifyJS.AST_Scope.prototype.next_mangled = function(options){
     ...
     while(mOptions.except.indexOf(m) != -1){
  ...
  */
  ...
  var result = "";
  if(options.banner){
    result += options.banner + LINE_BREAK;
  }
  result += stream;
  if(options.footer){
    result += LINE_BREAK + options.footer;
  }
  return result;
}
...
/**
@method minifyJSON
@param {String} filecontent JSON代码
@param {Object} options 压缩配置项
    @param {int} options.indent 字符缩进，默认0，最大形式压缩
@example
    
    var jsoncontent = fileutil.read('my.json');
    var minifycontent = this.minifyJSON(jsoncontent);

**/
exports.minifyJSON = function(filecontent, options){
  options = merge({indent: 0});
  var json = JSON.parse(filecontent);
  var content = JSON.stringify(json, null, options.indent);
  if(options.banner){
    content = options.banner + LINE_BREAK + content;
  }
  if(options.footer){
    content = content + LINE_BREAK + options.footer;
  }
  return content;
}
...
exports.minifyCSS = function (filecontent, options) {
  ...
  if(options.banner){
    filecontent = options.banner + LINE_BREAK + filecontent;
  }
  if(options.footer){
    filecontent = filecontent + LINE_BREAK + options.footer;
  }
...
exports.minifyImage = function(input, output, callback, options){
  ...
  }, options.level, options.verbose, options);
}
/**
@method smushit
@param {String} input 图片输入地址
...
exports.minifyHTML = function (filecontent, options) {
  ...
  var content = minify_html(filecontent, options)
  if(options.banner){
    content = options.banner + LINE_BREAK + content;
  }
  if(options.footer){
...
@param {String} filecontent css代码
@param {Object} options 压缩配置项
    @param {String} options.input css的当前路径
    @param {String} options.workspace css中定义的图片路径的绝对路径
@example
...
exports.datauri = function (filecontent, options) {
  ...
  filecontent = filecontent.replace(REGEXP_BACKGROUND, function (matches, prefix, urls, postfix) {
    ...
    var result = urls.replace(REGEXP_IMAGE, function (imageMatches, basename, type) {
      ...
      if(options.maxSize && base64.length > options.maxSize){
        return imageMatches;
      }
