...
@option {String} inputFile 输入文件路径
@option {String} outputFile 输出文件路径
@option {String} level
@option {Boolean} verbose 是否输出debug信息
@option {Function} done 优化完成后的函数
**/
exports.optimage = function(inputFile, outputFile, level, verbose, done){
    ...
    switch (extname){
        ...
        default:
            return done({'code': 400, message: 'Cannot support this extension ' + extname});
    }
    ...
    execFile(binPath, args, function(err, stdout, stderr) {
        ...
        if(err){
          return done({code: 500, message: err.toString()});
        }
        
        var saved = originalSize - fs.statSync(outputFile).size;
        if(saved == 0){
          return done({code: 300, message: 'No saved.'});
        }
        

        done(null, {input:inputFile, output:outputFile, saved: saved});
    });
...
@example
    ...
    this.minifyImageLocal('logo.png', 'logo.png', function(e, data){
        if(e){
          console.log(e);
        }else{
          console.log(data.msg);
...
**/
exports.minifyImageRemote = function (input, output, callback, options) {
  this.minifyImageLocal(input, output, callback, 
}
/**
@method minifyImageRemote
@param {String} input 图片输入地址
@param {String} output 图片输出地址
@param {Object} options 配置参数
    @param {Boolean} options.service 压缩服务http地址，默认使用!Yahoo的smush.it服务
@example
    ...
    this.minifyImageRemote('logo.png', 'logo.png', function(e, data){
        if(e){
          console.log(e);
        }else{
          console.log(data.msg);
