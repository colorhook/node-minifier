...
var REGEXP_SELECTOR = /([\w\-]+\s*:\s*)(url\s*\(.+\)\s*[\w\s,\-]*)+(;|(?=\}))/gi;
var REGEXP_IMAGE = /url\(\s*["']?\s*([^\(\)]*)\.(cur|png|jpg|jpeg|gif)\s*["']?\s*\)/gi;
...
var ast_squeeze_console = function (ast, options) {
  ...
  return w.with_walkers({
    ...
    call: function (expr, args) {
      ...
      //console.log
      if (!isLog) {
        isLog = expr[1] instanceof Array && expr[1][0] == 'name' && 
...
exports.datauri = function (filecontent, dir, options) {
  ...
  filecontent = filecontent.replace(REGEXP_SELECTOR, function (matches, prefix, urls, postfix) {
    //获得图片的路径
